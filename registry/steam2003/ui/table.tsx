  ...
  return (
    <div className="flex [&_thead_tr:first-child>*:last-child]:[border-right:none] [&_tfoot_tr:last-child>*:last-child]:[border-right:none]">
      <div
        ref={containerRef}
        className={cn(
          "flex-1 min-w-0 overflow-x-auto overflow-y-scroll [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [scroll-snap-type:y_mandatory] [scroll-behavior:auto] [box-shadow:inset_1px_0_0_var(--border-dark),inset_0_-1px_0_var(--border-light)] outline-none focus-visible:win32-focus-primary",
          containerClassName
        )}
      >
        <table
...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn("sticky top-0 z-10 min-h-4", className)}
      {...props}
    />
  )
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "[scroll-snap-align:start] data-[state=selected]:bg-primary data-[state=selected]:text-primary-foreground",
        className
      )}
      {...props}
    />
